...
- **`$SKILL_SRC/bootstrapping/<skill>/`** — Sub-skills: `architecture-review`, `backlog`, `cloud-system-engineer`, `grafana`, `ideation`, `implement`, `sprint-review`, `visualize`
...
# Sub-skills live under bootstrapping/ in the repo
...
# Resolve repo path (bootstrapping/ vs. top-level) — see "Repo structure" above
BOOTSTRAPPING_SUBSKILLS="architecture-review backlog cloud-system-engineer grafana ideation implement sprint-review visualize"
if echo "$BOOTSTRAPPING_SUBSKILLS" | grep -qw "<skill>"; then
  SRC_PATH="$SKILL_SRC/bootstrapping/<skill>"
else
